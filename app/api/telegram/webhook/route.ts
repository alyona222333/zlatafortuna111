...
export async function POST(req: NextRequest) {
  try {
    ...
    // The Settings page registers a tenant-specific URL with ?bid=... .
    // Keep the parameter optional for a single-business deployment as well:
    // Telegram users commonly paste /api/telegram/webhook manually, without
    // knowing the internal business UUID. Never guess when multiple bots are
    // configured, because that could route a client's message to the wrong
    // tenant.
    ...
    if (businessId) {
      ...
      bizQuery = bizQuery.not('telegram_bot_token', 'is', null)
    }

    const { data: businesses, error: businessError } = await bizQuery.limit(2)
    if (businessError) {
      console.error('[telegram/webhook] business lookup failed', businessError)
    ...
    if (!businessId && businesses.length > 1) {
      console.error('[telegram/webhook] bid is required when multiple bots are configured')
    ...
    if (text.startsWith('/start')) {
      ...
      await sendTelegramMessage(
        ...
        [
          `👋 Hi ${firstName}!`,
          ``,
          `You are now connected to <b>${biz.name}</b> on Zlata Fortuna Group.`,
          ``,
    ...
    if (text.startsWith('/help')) {
      await sendTelegramMessage(
        ...
        chatId,
        [
          `<b>Zlata Fortuna Group Bot — available commands:</b>`,
          ``,
