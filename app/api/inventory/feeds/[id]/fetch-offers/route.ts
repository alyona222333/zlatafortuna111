...
export async function POST(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  try {
    const body = await request.json().catch(() => ({})) as { group_id?: string; sync_started_at?: string; finalize?: boolean }
    const report = await fetchFeedIntoCache(supabase, access.businessId, id, {
      groupId: typeof body.group_id === 'string' ? body.group_id : undefined,
      syncStartedAt: typeof body.sync_started_at === 'string' ? body.sync_started_at : undefined,
