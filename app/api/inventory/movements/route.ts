...
export async function GET(request: Request) {
  ...
  let query = supabase.from('inventory_movements')
    .select('id, item_id, type, quantity, note, created_at, inventory_items!inner(name, sku, unit, source, is_archived)')
    .eq('business_id', business.id)
    .eq('inventory_items.source', 'site')
    .eq('inventory_items.is_archived', false)
...
export async function POST(request: Request) {
  ...
  const business = await getBusiness(supabase, user.id)
  if (!business) return NextResponse.json({ error: 'business_not_found' }, { status: 404 })
  const body = await request.json().catch(() => ({})) as { item_id?: string; operation?: string; quantity?: number; note?: string; actual_quantity?: number; from_location?: string; to_location?: string; payment_method?: string; order_reference?: string }
  if (!body.item_id) return NextResponse.json({ error: 'item_id_required' }, { status: 400 })
  const { data: item, error: itemError } = await supabase.from('inventory_items').select('id, quantity, unit, name').eq('id', body.item_id).eq('business_id', business.id).eq('source', 'site').eq('is_archived', false).single()
  ...
  if (operation === 'return') {
    ...
    if (/налож|післяплат|післяплата|cash.?on.?delivery|cod/i.test(`${paymentMethod} ${noteText}`)) riskReasons.push('наложенный платеж')
    if (!body.order_reference) riskReasons.push('нет номера заказа')
    if (Number(quantity) >= 3) riskReasons.push('крупное количество')
    const riskLevel = riskReasons.length ? 'suspicious' : 'normal'
    const admin = createServiceClient() as any
    await admin.from('return_security_events').insert({ business_id: business.id, actor_id: user.id, item_id: item.id, operation, payment_method: body.payment_method ?? null, order_reference: body.order_reference ?? null, quantity: Math.abs(quantity), note: noteText || null, risk_level: riskLevel, risk_reasons: riskReasons })
    if (riskLevel === 'suspicious') {
      const { data: channel } = await admin.from('internal_channels').select('id').eq('business_id', business.id).eq('slug', 'hr-security').maybeSingle()
      if (channel?.id) await admin.from('internal_messages').insert({ business_id: business.id, channel_id: channel.id, sender_id: user.id, body: `ПОДОЗРИТЕЛЬНЫЙ ВОЗВРАТ: ${item.name}; количество ${quantity}; заказ ${body.order_reference || 'не указан'}; способ оплаты ${body.payment_method || 'не указан'}; причины: ${riskReasons.join(', ')}. Требуется проверка службой безопасности.` })
    }
  }
