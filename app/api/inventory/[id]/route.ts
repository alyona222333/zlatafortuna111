...
export async function PATCH(request: Request, props: { params: Promise<{ id: string }> }) {
  ...
  const manualKeys = ['name', 'category', 'description', 'long_description', 'product_url', 'image_url', 'sell_price', 'site_regular_price', 'site_sale_price', 'site_short_description', 'site_attributes']
  ...
  const { data, error } = await supabase.from('inventory_items').update({
    name: body.name,
    // SKU — идентификатор каталога и всегда остаётся прежним.
