  ...
  async function sendInvite(employeeId: string) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setInviteLinks((prev) => ({ ...prev, [employeeId]: json.inviteUrl }))
      }
    } finally {
      setInvitingId(null)
    }
                    ...
                    <tr key={e.id} className="border-b border-gray-100 hover:bg-gray-50 last:border-0">
                      ...
                      <td className="px-4 py-3">
                        {e.user_id ? (
                          ...
                          <button
                            onClick={() => { navigator.clipboard.writeText(inviteLinks[e.id]); }}
                            className="text-xs text-blue-600 underline"
                            title={inviteLinks[e.id]}
                          >
                            {locale === 'ru' ? 'Копировать ссылку' : locale === 'en' ? 'Copy invite link' : 'Копіювати посилання'}
                          </button>
                        ) : (
                          <button
