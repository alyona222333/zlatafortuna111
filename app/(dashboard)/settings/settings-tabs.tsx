...
interface Employee {
  id: string; name: string; role: string; email: string | null; phone: string | null; is_active: boolean
  user_id?: string | null; invite_sent_at?: string | null; invite_accepted_at?: string | null
}
...
interface Props {
  ...
  services: Service[]; employees: Employee[]; workingHours: DayHours[]; userEmail: string
  isOwner?: boolean
  allowedTabs?: string[] | null // null/undefined = no restriction (owner/director)
}
type Tab = 'general' | 'services' | 'employees' | 'notifications' | 'billing' | 'account' | 'modules'

export function SettingsTabs({ business: initial, services: initServices, employees: initEmployees, workingHours: initHours, userEmail, isOwner = true, allowedTabs = null }: Props) {
  ...
  const initialTab = (['general', 'services', 'employees', 'notifications', 'billing', 'modules'].includes(searchParams.get('tab') ?? '')
    && (!allowedTabs || allowedTabs.includes(searchParams.get('tab') as string))
    ? searchParams.get('tab')
    : (allowedTabs?.[0] ?? 'general')) as Tab
  ...
  const [initialBizSnapshot] = useState(initial)
  ...
  async function sendInvite(employeeId: string) {
    ...
    setInviteError((prev) => ({ ...prev, [employeeId]: '' }))
    try {
      ...
      if (!res.ok) {
        setInviteError((prev) => ({ ...prev, [employeeId]: json.error ?? 'Error' }))
      } else {
        setInviteLinks((prev) => ({ ...prev, [employeeId]: String(json.inviteUrl ?? '').replace(/^URL=/i, '').trim() }))
      }
    } finally {
      setInvitingId(null)
    }
  ...
  const [enabledModules, setEnabledModules] = useState<string[]>(initial.enabled_modules ?? DEFAULT_MODULES)
  const [initialModulesSnapshot] = useState<string[]>(initial.enabled_modules ?? DEFAULT_MODULES)
  ...
  const [initialHoursSnapshot] = useState<DayHours[]>(() => DEFAULT_HOURS.map((def) => initHours.find((h) => h.day_of_week === def.day_of_week) ?? def))
  ...
  function resetGeneralChanges() {
    setBiz(initialBizSnapshot)
    setLogoUrl(initialBizSnapshot.logo_url ?? null)
    setSlugError('')
  }

  function resetHoursChanges() {
    setHours(initialHoursSnapshot.map((day) => ({ ...day })))
    setHoursValidationError(null)
  }
  ...
  async function deleteEmployee(id: string) {
    ...
    if (!res.ok) {
      ...
      setInviteError((prev) => ({ ...prev, [id]: json.error ?? 'Не вдалося видалити співробітника' }))
      return
    }
    setEmployees((prev) => prev.filter((e) => e.id !== id))
    setInviteLinks((prev) => { const next = { ...prev }; delete next[id]; return next })
    setInviteError((prev) => { const next = { ...prev }; delete next[id]; return next })
  ...
  const tabs = allowedTabs ? allTabs.filter((tb) => allowedTabs.includes(tb.key)) : allTabs
      ...
      {tab === 'general' && (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <Button variant="outline" onClick={resetGeneralChanges} disabled={saving}>{common('actions.cancel')}</Button>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="mt-5 flex items-center gap-3">
            ...
            <Button variant="outline" onClick={resetHoursChanges} disabled={savingHours}>{common('actions.cancel')}</Button>
        ...
        <div className="space-y-4">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50 text-xs text-gray-500 uppercase">
                    ...
                    <th className="text-left px-4 py-3 font-medium">Вхід</th>
                    <th className="px-4 py-3" />
                  </tr>
                </thead>
                <tbody>
                  {employees.map((e) => (
                    <tr key={e.id} className="border-b border-gray-100 hover:bg-gray-50 last:border-0">
                      ...
                      <td className="px-4 py-3 text-gray-500 hidden sm:table-cell min-w-[9rem]">
                        {ROLE_LABELS[e.role as RoleSlug]?.[locale === 'ru' ? 'ru' : locale === 'en' ? 'en' : 'uk'] ?? e.role}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {e.user_id ? (
                          <span className="inline-flex items-center gap-1 text-xs text-green-700 bg-green-50 px-2 py-1 rounded">
                            <CheckCircle2 className="w-3 h-3" /> Активний
                          </span>
                        ) : inviteLinks[e.id] ? (
                          <button
                            onClick={async () => { const cleanUrl = String(inviteLinks[e.id] ?? '').replace(/^URL=/i, '').trim(); await navigator.clipboard.writeText(cleanUrl); setCopiedInviteId(e.id); window.setTimeout(() => setCopiedInviteId((current) => current === e.id ? null : current), 1800); }}
                            className="text-xs text-blue-600 underline"
                            title={inviteLinks[e.id]}
                          >
                            {copiedInviteId === e.id ? (locale === 'ru' ? 'Скопировано' : locale === 'en' ? 'Copied' : 'Скопійовано') : (locale === 'ru' ? 'Копировать ссылку' : locale === 'en' ? 'Copy invite link' : 'Копіювати посилання')}
                          </button>
                        ) : (
                          <button
                            ...
                            title={!e.email ? 'Спочатку вкажіть email' : 'Створити запрошення для входу'}
                          >
                            {invitingId === e.id ? (locale === 'ru' ? 'Создание…' : locale === 'en' ? 'Creating…' : 'Створення…') : (locale === 'ru' ? 'Пригласить' : locale === 'en' ? 'Invite' : 'Запросити')}
                          </button>
                        )}
                        {inviteError[e.id] && <div className="text-[11px] text-red-500 mt-1">{inviteError[e.id]}</div>}
                      </td>
                      <td className="px-4 py-3 text-right whitespace-nowrap">
                        {confirmDeleteEmpId === e.id ? (
                          ...
                          <div className="flex justify-end gap-1">
                            ...
                            <button onClick={() => setConfirmDeleteEmpId(e.id)} title={locale === 'ru' ? 'Удалить сотрудника' : locale === 'en' ? 'Delete employee' : 'Видалити співробітника'} aria-label={locale === 'ru' ? 'Удалить сотрудника' : locale === 'en' ? 'Delete employee' : 'Видалити співробітника'} className="inline-flex items-center gap-1 px-2 py-1.5 text-xs text-red-600 hover:bg-red-50 rounded"><Trash2 className="w-3.5 h-3.5" />{locale === 'ru' ? 'Удалить' : locale === 'en' ? 'Delete' : 'Видалити'}</button>
                          </div>
                        )}
                      </td>
                    </tr>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-xs font-medium text-gray-500">{t('employees.fields.role')}</label>
                <select
                  value={empForm.role ?? 'employee'}
                  onChange={(e) => setEmpForm((f) => ({ ...f, role: e.target.value }))}
                  className="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
                  {ROLE_SLUGS.map((slug) => (
                    <option key={slug} value={slug}>{ROLE_LABELS[slug][locale === 'ru' ? 'ru' : locale === 'en' ? 'en' : 'uk']}</option>
                  ))}
                </select>
              </div>
            </div>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">
              Це власна CRM Zlata Fortuna Group. Всі функції доступні без підписки.
            </p>
      ...
      {tab === 'modules' && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
            ...
            <button type="button" onClick={() => setEnabledModules([...initialModulesSnapshot])} disabled={modulesSaving} className="inline-flex items-center gap-2 rounded-lg border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50">
