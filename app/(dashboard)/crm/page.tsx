...
export default async function CRMPage(
  ...
  // All distinct tags used across this business's clients, for the filter
  // chips below the search bar (lightweight equivalent of the reference
  // CRM's "Теги" list — no separate tags table needed since tags already
  // live on each client as a text array).
  const { data: allClientsForTags } = await supabase
    .from('clients')
    .select('tags')
    .eq('business_id', business.id)
    .not('tags', 'is', null)
  const allTags = Array.from(new Set((allClientsForTags ?? []).flatMap((c) => c.tags ?? []))).sort()
      ...
      <main className="p-6">
        ...
        {allTags.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-4">
            <Link href="/crm">
              <span className={`text-xs px-2.5 py-1 rounded-full border ${!searchParams.tag ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-300 text-gray-600'}`}>
                Всі
              </span>
            </Link>
            {allTags.map((tag) => (
              <Link key={tag} href={`/crm?tag=${encodeURIComponent(tag)}`}>
                <span className={`text-xs px-2.5 py-1 rounded-full border ${searchParams.tag === tag ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-300 text-gray-600'}`}>
                  {tag}
                </span>
              </Link>
            ))}
