...
import { getMyEmployeeRole } from '@/lib/business'
...
export default async function DashboardPage() {
  ...
  // The onboarding flag belongs to the owner. An invited employee is already
  // attached to an existing business and must not be sent through the owner's
  // onboarding wizard, otherwise /dashboard and /onboarding loop forever.
  const isOwner = business.owner_id === user!.id
  if (isOwner && !business.onboarding_completed) redirect('/onboarding')

  const role = isOwner ? null : await getMyEmployeeRole(user!.id)
  // Revenue and operational counters are limited to leadership, accounting,
  // and the service-sales team. Other employees get a welcoming work hub
  // without prices, amounts, stock or client totals.
  const detailedDashboardRoles = new Set(['director', 'accountant', 'sales_manager_services'])
  if (!isOwner && !detailedDashboardRoles.has(role ?? '')) {
    return <GeneralDashboard businessName={business.name} />
  }

  // Financial figures (today's revenue, recent sales amounts) are only for
  // the owner or an employee with role 'director' — everyone else gets the
  // same dashboard minus money. The database enforces this too (migration
  // 039); this just keeps the UI honest instead of showing a misleading
  // partial/zero number to someone RLS is quietly filtering.
  const canSeeFinance = isOwner || detailedDashboardRoles.has(role ?? '')
    ...
    <>
      ...
      <main className="p-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Revenue card — custom render for sparkline. Owner/director only. */}
          {canSeeFinance && (
