...
export function SuppliersView() {
  ...
  async function loadOffers() {
    ...
    try {
      const res = await fetch(`/api/inventory/offers?q=${encodeURIComponent(offerQuery)}`)
  ...
  async function sync(feedId?: string) {
    ...
    try {
      ...
      const targets = feedId
        ? feeds.filter((f) => f.id === feedId && f.is_active)
        : feeds.filter((f) => f.is_active)
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-gray-500 mt-2">
                      {t('feeds.stats', {
                        offers: feed.last_offers_count,
                        matched: feed.last_matched_count,
                        matchedOffers: feed.last_matched_offers_count,
                        unmatched: Math.max(0, feed.last_offers_count - feed.last_matched_offers_count),
                        when: fmtDate(feed.last_synced_at),
                      })}
                    </div>
                    {feed.last_error && (
      ...
      <section className="bg-white border border-gray-200 rounded-xl p-5">
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-sm text-gray-500">Офери постачальників, яких ще немає у каталозі сайту. Додавайте лише вибрані товари — по одному.</p>
          </div>
