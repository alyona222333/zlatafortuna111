...
export default async function InventoryItemPage(props: { params: Promise<{ id: string }> }) {
  ...
  const { data: item } = await (supabase
    ...
    .select('id, name, sku, barcode, category, unit, quantity, low_stock_threshold, sell_price, description, long_description, product_url, image_url, site_regular_price, site_sale_price, site_short_description, site_attributes, created_at, updated_at')
    .eq('id', params.id)
