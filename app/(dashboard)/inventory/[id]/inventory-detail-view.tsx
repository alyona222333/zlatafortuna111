...
export function InventoryDetailView({ item: initial, movements: initialMovements, currency, timezone, businessId, categories }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    site_regular_price: String(initial.site_regular_price ?? ''),
    site_sale_price: String(initial.site_sale_price ?? ''),
    site_short_description: initial.site_short_description ?? '',
    site_attributes: initial.site_attributes ?? '',
  })
  ...
  async function saveItem() {
    ...
    const res = await fetch(`/api/inventory/${item.id}`, {
      ...
      body: JSON.stringify({
        ...
        site_regular_price: form.site_regular_price,
        site_sale_price: form.site_sale_price,
        site_short_description: form.site_short_description,
        site_attributes: form.site_attributes,
      }),
    })
    if (res.status === 409) {
  ...
  async function uploadPhoto(file: File) {
    ...
    try {
      ...
      const res = await fetch(`/api/inventory/${item.id}/photo`, { method: 'POST', body })
      const data = await res.json()
      if (!res.ok) {
        setPhotoError(data.error ?? 'Помилка завантаження')
        return
      }
      setForm((f) => ({ ...f, image_url: data.url }))
      setItem((i) => ({ ...i, image_url: data.url } as typeof i))
    } catch {
      setPhotoError('Помилка завантаження')
    } finally {
            ...
            {editing ? (
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <input type="text" value={form.sku} readOnly disabled
                    title="Артикул не можна змінювати: за ним склад звіряється з прайсами постачальників"
                    className="w-full mt-1 border border-gray-200 bg-gray-50 text-gray-500 rounded-lg px-3 py-2 text-sm cursor-not-allowed" />
                  <p className="mt-1 text-xs text-gray-400">SKU закріплений за товаром і не редагується — він використовується для імпорту та експорту каталогу.</p>
                ...
                {([
                  { key: 'sell_price', label: 'Ціна сайту', type: 'number' },
                  { key: 'site_regular_price', label: 'Стара ціна сайту', type: 'number' },
                  { key: 'site_sale_price', label: 'Акційна ціна сайту', type: 'number' },
                ...
                {([
                  ['site_short_description', 'Короткий опис'],
                  ['description', 'Опис'],
                  ['long_description', 'Довгий опис'],
                  ['site_attributes', 'Параметри'],
                  ['product_url', 'Посилання на товар'],
                ] as const).map(([key, label]) => (
                  <label key={key} className="block text-xs font-medium text-gray-500">
                    {label}
                    <textarea value={form[key]} onChange={(e) => setForm((f) => ({ ...f, [key]: e.target.value }))} rows={key.includes('description') ? 3 : 1} className="w-full mt-1 border border-gray-200 rounded-lg px-3 py-2 text-sm font-normal" />
                  </label>
                ))}

                <div>
                  <label className="text-xs font-medium text-gray-500">Фото товару</label>
                  <div className="mt-1 flex items-center gap-3">
                    {form.image_url && (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={form.image_url} alt="" className="w-14 h-14 rounded-lg object-cover border border-gray-200" />
                    )}
                    <div className="flex-1 space-y-1">
                      <input type="file" accept="image/jpeg,image/png,image/webp"
                        disabled={uploadingPhoto}
                        onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadPhoto(f) }}
                        className="block w-full text-xs text-gray-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border file:border-gray-200 file:text-xs file:bg-white hover:file:bg-gray-50" />
                      <input type="text" value={form.image_url} placeholder="або встав посилання на фото"
                        onChange={(e) => setForm((f) => ({ ...f, image_url: e.target.value }))}
                        className="w-full border border-gray-200 rounded-lg px-3 py-1.5 text-xs" />
                      {uploadingPhoto && <p className="text-xs text-gray-400">Завантаження…</p>}
              ...
              <div className="space-y-2 text-sm">
                ...
                {[
                  ...
                  { label: 'Ціна сайту', value: item.sell_price != null ? formatCurrency(item.sell_price, currency) : null },
                  { label: 'Звичайна ціна сайту', value: item.site_regular_price != null ? formatCurrency(item.site_regular_price, currency) : null },
                  { label: 'Акційна ціна сайту', value: item.site_sale_price != null ? formatCurrency(item.site_sale_price, currency) : null },
