...
export function InventoryDetailView({ item: initial, movements: initialMovements, currency, timezone, businessId, categories }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    dropship_price: String(initial.dropship_price ?? ''),
  ...
  async function saveItem() {
    ...
    const res = await fetch(`/api/inventory/${item.id}`, {
      ...
      body: JSON.stringify({
        ...
        dropship_price: form.dropship_price,
            ...
            {editing ? (
              <div className="space-y-3">
                ...
                {([
                  ...
                  { key: 'dropship_price', label: 'Оптова ціна дропшипера', type: 'number' },
                  ...
                  { key: 'supplier_name', label: 'Постачальник', type: 'text' },
                  { key: 'supplier_phone', label: 'Телефон постачальника', type: 'tel' },
              ...
              <div className="space-y-2 text-sm">
                ...
                {[
                  ...
                  { label: 'Ціна дропшипера', value: item.dropship_price != null ? formatCurrency(item.dropship_price, currency) : null },
                  { label: 'Опт постачальника', value: item.supplier_price != null ? formatCurrency(item.supplier_price, currency) : null },
                  { label: 'РРЦ постачальника', value: item.supplier_retail_price != null ? formatCurrency(item.supplier_retail_price, currency) : null },
                  { label: t('fields.lowStockThreshold'), value: `${item.low_stock_threshold} ${item.unit}` },
                  { label: 'Постачальник', value: item.supplier_name },
                  { label: 'Телефон постачальника', value: item.supplier_phone },
