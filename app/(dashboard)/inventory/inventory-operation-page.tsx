...
export async function InventoryOperationPage({ operation, title, historyKind }: { operation: Operation; title: string; historyKind?: string }) {
  ...
  const [{ data: items }, { data: movements }] = await Promise.all([
    supabase.from('inventory_items').select('id, name, sku, quantity, unit').eq('business_id', business.id).order('name'),
    supabase.from('inventory_movements').select('id, item_id, type, quantity, note, created_at, inventory_items(name, sku, unit)').eq('business_id', business.id).order('created_at', { ascending: false }).limit(100),
  ])
