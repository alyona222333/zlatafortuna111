...
export function InventoryOperationView({ operation, items, initialMovements }: { operation: Operation; items: Item[]; initialMovements: Movement[] }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch('/api/inventory/movements', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ item_id: itemId, operation, quantity: Number(quantity), actual_quantity: Number(actual), note, from_location: from, to_location: to }) })
    const data = await res.json()
    setSaving(false)
    if (!res.ok) { setMessage(data.error === 'quantity_exceeds_stock' ? t('insufficient') : data.error ?? t('noRecords')); return }
    setMessage(t('operationSaved', { quantity: data.quantity })); setQuantity(''); setActual(''); setNote(''); setTo('')
