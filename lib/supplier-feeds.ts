...
export const FEED_FETCH_TIMEOUT_MS = 120_000
...
export async function fetchFeed(url: string): Promise<string> {
  ...
  try {
    ...
    if (len && len > MAX_FEED_BYTES) {
      throw new Error(`Файл завеликий (${Math.round(len / 1048576)} МБ)`)
    }

    const text = await res.text()
    if (!text.includes('<offer')) {
      throw new Error('У відповіді немає жодного <offer> — перевірте посилання')
    }
    return text
  } finally {
    clearTimeout(timer)
...
export type MatchType = 'sku' | 'barcode' | 'name'
...
export function matchItemsToOffers(
  ...
  const skuDupes = new Set<string>()
  const byBarcode = new Map<string, MatchableOffer>()
  const byName = new Map<string, MatchableOffer>()
  const nameDupes = new Set<string>()
  const byProductCode = new Map<string, MatchableOffer>()
  const productCodeDupes = new Set<string>()

  for (const o of offers) {
    const sku = normArticle(o.vendor_code)
    if (sku) {
      if (bySku.has(sku)) {
        // Той самий артикул у двох постачальників: лишаємо той, де товар є
        // в наявності — менеджеру потрібно знати, що взяти можна.
        const prev = bySku.get(sku)!
        if (!prev.available && o.available) bySku.set(sku, o)
        else skuDupes.add(sku)
      } else {
        bySku.set(sku, o)
      }
    }
    if (o.barcode) {
      ...
      if (bc && !byBarcode.has(bc)) byBarcode.set(bc, o)
    }
    const nm = normName(o.name)
    if (nm) {
      if (byName.has(nm)) nameDupes.add(nm)
      else byName.set(nm, o)
    }
    for (const code of productCodes(`${o.vendor_code ?? ''} ${o.name ?? ''}`)) {
      if (byProductCode.has(code)) productCodeDupes.add(code)
      else byProductCode.set(code, o)
    }
  }
  for (const n of nameDupes) byName.delete(n)
  for (const code of productCodeDupes) byProductCode.delete(code)

  // Назва, що повторюється в нас на сайті, теж непридатна як ключ.
  const itemNameCount = new Map<string, number>()
  for (const it of items) {
    const nm = normName(it.name)
    if (nm) itemNameCount.set(nm, (itemNameCount.get(nm) ?? 0) + 1)
  }
  ...
  for (const it of items) {
    ...
    if (it.barcode) {
      const hit = byBarcode.get(it.barcode.trim())
      if (hit) {
        matches.push({ item_id: it.id, offer: hit, match_type: 'barcode' })
        continue
      }
    }
    const codeHits = productCodes(`${it.sku ?? ''} ${it.name ?? ''}`)
      .map((code) => byProductCode.get(code)).filter(Boolean) as MatchableOffer[]
    if (codeHits.length === 1) {
      matches.push({ item_id: it.id, offer: codeHits[0], match_type: 'name' })
      continue
    }
    const nm = normName(it.name)
    if (nm && itemNameCount.get(nm) === 1) {
      const hit = byName.get(nm)
      if (hit) {
        matches.push({ item_id: it.id, offer: hit, match_type: 'name' })
        continue
      }
    }
