...
export async function runWarehouseSync(
  activeFeeds: { id: string; name: string; url?: string | null }[],
  ...
  for (let i = 0; i < activeFeeds.length; i++) {
    ...
    try {
      const groups = feed.url ? new URL(feed.url).searchParams.get('group_ids')?.split(',').map((v) => v.trim()).filter(Boolean) ?? [] : []
      const startedAt = new Date().toISOString()
      const reports: FeedSyncReport[] = []
      for (let g = 0; g < Math.max(groups.length, 1); g++) {
        reports.push(await postJson<FeedSyncReport>(
          `/api/inventory/feeds/${feed.id}/fetch-offers`,
          groups.length ? { group_id: groups[g], sync_started_at: startedAt, finalize: g === groups.length - 1 } : {},
        ))
      }
      feeds.push({
        ...reports[reports.length - 1],
        offers: reports.reduce((sum, report) => sum + report.offers, 0),
        skipped: reports.reduce((sum, report) => sum + report.skipped, 0),
        status: reports.some((report) => report.status === 'error') ? 'error' : 'ok',
      })
    } catch (e) {
      feeds.push({
        feed_id: feed.id,
  ...
  const successfulFeedIds = feeds.filter((feed) => feed.status === 'ok').map((feed) => feed.feed_id)
  const match = await postJson<MatchReport>('/api/inventory/feeds/match', { feed_ids: successfulFeedIds })
