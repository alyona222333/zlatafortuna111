...
export async function fetchFeedIntoCache(
  ...
  businessId: string,
  feedId: string,
  options: { groupId?: string; syncStartedAt?: string; finalize?: boolean } = {},
  ...
  // Позначаємо статус ДО довгого мережевого запиту, а не тільки в catch:
  // якщо хостинг сам вб'є функцію по своєму ліміту часу, жоден JS catch не
  // встигне виконатись, і без цього запису картка постачальника застигла б
  // на старому «Працює» з давньою датою, хоча оновлення обірвалось.
  await supabase
    .from('supplier_feeds')
    .update({
      last_status: 'error',
      last_error: 'Оновлення виконується або було перерване — якщо цей статус не зміниться, постачальник не встиг відповісти вчасно.',
    } as never)
    .eq('id', f.id)
  ...
  try {
    ...
    if (options.groupId) {
      ...
      parsedUrl.searchParams.set('group_ids', options.groupId)
    ...
    // Видаляємо старі позиції лише після останнього пакетного запиту.
    if (options.finalize !== false) {
      await supabase.from('supplier_offers').delete().eq('feed_id', f.id).lt('fetched_at', options.syncStartedAt ?? startedAt)
    }
    ...
    await supabase
      ...
      .update({
        ...
        last_status: 'ok',
        last_error: null,
        last_offers_count: cachedCount ?? rows.length,
      } as never)
      .eq('id', f.id)

    return { feed_id: f.id, name: f.name, url: f.url, status: 'ok', offers: cachedCount ?? rows.length, skipped: parsed.skipped }
  } catch (e) {
...
export async function matchWarehouseAgainstOffers(
  ...
  const { data: feedsRaw } = await supabase
    ...
    .select(
      ...
      'notify_out_of_stock, last_status, default_stock_when_available, default_markup_percent',
    )
    .eq('business_id', businessId)
    .eq('is_active', true)
  ...
  const loadItems = () =>
    selectAll<ItemBefore>((from, to) =>
      supabase
        ...
        .select('id, sku, barcode, name, product_url, source, track_supplier, supplier_available, supplier_offer_id, supplier_feed_id')
        .eq('business_id', businessId)
  ...
  const workingFeedIds = successfulFeedIds
    ? successfulFeedIds
    : feeds.filter((feed) => feed.last_status === 'ok').map((feed) => feed.id)
  const offers = workingFeedIds.length === 0 ? [] : await selectAll<MatchableOffer & { picture: string | null }>((from, to) =>
    supabase
      .from('supplier_offers')
  ...
  const { matches, unmatched: rawUnmatched } = matchItemsToOffers(items, offers)
  const failedFeedIds = new Set(feeds.map((feed) => feed.id).filter((id) => !workingFeedIds.includes(id)))
  const unmatched = rawUnmatched.filter((itemId) => {
    const previous = items.find((item) => item.id === itemId)
    return !previous?.supplier_feed_id || !failedFeedIds.has(previous.supplier_feed_id)
  })
  ...
  for (const feed of feeds.filter((candidate) => workingFeedIds.includes(candidate.id))) {
    ...
    perFeedOffers.set(feed.id, feedOffers.filter((offer) => {
      ...
      return !!sku && siteSkuKeys.has(sku)
    }).length)
  }
  for (const feed of feeds.filter((candidate) => workingFeedIds.includes(candidate.id))) {
