...
export function InventoryExportButton() {
  ...
  async function handleExport() {
    setLoading(true)
    try {
      const res = await fetch(`/api/inventory/export?columns=${encodeURIComponent(selected.join(','))}`)
      if (!res.ok) return
      const blob = await res.blob()
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a'); a.href = url
      const match = (res.headers.get('Content-Disposition') ?? '').match(/filename="([^"]+)"/)
      a.download = match ? match[1] : 'pronto-woocommerce.csv'; a.click(); URL.revokeObjectURL(url)
  ...
  return <div className="relative">
    <div className="flex gap-1">
      ...
      <Button variant="outline" size="sm" onClick={handleExport} disabled={loading || !selected.length}><Download className="w-4 h-4 mr-1" />{loading ? '…' : t('export')}</Button>
    </div>
